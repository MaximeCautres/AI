""" Main program -- manage UI and deep neural network """
...
if save_name is None:
    ...
    parameters = dnn.initialize_parameters(layers_dims, activations)
    arguments = (parameters, epoch_count, print_rate, mini_batch_size, alpha,
                 ad_rate, beta1, beta2, lambda2, keep_dims, training, testing, take_best)
    parameters, name, best_cost = dnn.train(*arguments)

    if save_training:
        hyper_parameters = {'layers_dims': layers_dims, 'activations' : activations, 'take_best' : take_best,
                            'epoch_count': epoch_count, 'mini_batch_size': mini_batch_size,
                            'alpha': alpha, 'ad_rate': ad_rate, 'beta1': beta1, 'beta2': beta2,
                            'lambda2' : lambda2, 'keep_dims' : keep_dims, 'best_cost' : best_cost}
        save = {'parameters' : parameters, 'hyper_parameters' : hyper_parameters}
        pickle.dump(save, open(name + '.p', 'wb'))

else:
    save = pickle.load(open(save_name + '.p', 'rb'))
    parameters = save['parameters']
    print(save['hyper_parameters'])

    # dnn.generate(parameters, goal, iteration_count, galpha, refresh_rate)
...
class Number:
    ...
    def analyze(self):
        ...
        self.guess = dnn.guess(parameters, request)
