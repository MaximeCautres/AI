...
class Network:
    ...
    def __init__(self, dim):
        ...
        self.weights = [np.create(dim[x], dim[x - 1]) for x in range(self.layers)]
        self.biases = [np.create(1, x) for x in dim]

        for i in range(self.layers):
            self.weights[i].randomize()
            self.biases[i].randomize()

        self.effect_weight = [np.create(dim[x], dim[x - 1]) for x in range(self.layers)]
        self.effect_bias = [np.create(1, x) for x in dim]

        self.z = [np.create(1, x) for x in dim]
        self.a = [np.create(1, x) for x in dim]
    ...
    def update(self, batch, learning_rate):
        ...
        stack_weight = [np.create(self.dim[x], self.dim[x - 1]) for x in range(self.layers)]
        stack_bias = [np.create(1, x) for x in self.dim]
        ...
        for layer in range(1, self.layers):
            stack_weight[layer] = stack_weight[layer].hadamard(ratio)
            self.weights[layer] = self.weights[layer].add(stack_weight[layer].hadamard(learning_rate))

            stack_bias[layer] = stack_bias[layer].hadamard(ratio)
            self.biases[layer] = self.biases[layer].add(stack_bias[layer].hadamard(learning_rate))
    ...
    def back_propagation(self, inp, out):
        ...
        delta = np.convert(out).subtract(self.a[-1]).hadamard(self.z[-1].sigmoid_prime())
    ...
    def success_count(self):
        ...
        for couple in test:
            ...
            if self.a[-1].smooth().matrix == [couple[1]]:
                count += 1
    ...
    """ Return the result when entry is the input """
